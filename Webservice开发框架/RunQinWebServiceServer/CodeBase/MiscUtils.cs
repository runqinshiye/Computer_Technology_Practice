...
namespace CodeBase
{
    public static class MiscUtils
    {
        ...
        public static Exception GetExceptionContainingMessage(Exception ex, string message, int depth = 0)
        {
            ...
            if (ex.InnerException == null || depth > 100)
            {//Limit to 100 inner exceptions to prevent stack overflow
                return null;
            }
        ...
        public static void DebugLogInterval(string locationName)
        {
            try
            {
                ...
                {//first time on each run
                    Debug.Write("----");
                }
                else if (locationName.StartsWith("-"))
                {//a quick convention for not showing a time that we don't care about.
                    Debug.Write("----");
                }
                else
                {
