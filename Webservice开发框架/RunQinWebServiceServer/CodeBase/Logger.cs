...
namespace CodeBase
{
    ...
    public partial class Logger
    {
        ...
        ///Should generally be used on one line statements.</summary>
        public static void LogActionIfOverTimeLimit(string log, LogPath path, Action act, int milliseconds = 5000)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            act();
            sw.Stop();
            if (sw.Elapsed > TimeSpan.FromMilliseconds(milliseconds))
            {
                LogToPath(log, path, LogPhase.Unspecified, $"Took too long: {(int)sw.Elapsed.TotalSeconds} seconds");
            }
        }
        ...
        public static void LogVerbose(string log, string subDirectory = "")
        {
            try
            {
                ...
                if (DateTime.Now.Subtract(_lastLoggerCleanup) > TimeSpan.FromDays(1))
                { //Once a day logger cleanup is due.
            ...
            , Action onExit = null, int cleanIfOlderThanDays = 30)
        {
            ...
            if (_threadLoggerCleanup != null)
            { //Still running from 1 day ago. That would be a problem.
                return onCancel;
            }
            ...
            try
            {
                ...
                deleteEmptyDir = new Action<string>((diCur) =>
                {
                    ...
                    try
                    {
                        ...
                        foreach (string d in Directory.GetDirectories(diCur, "*.*", SearchOption.TopDirectoryOnly))
                        { //All directories that should delete must delete in order to return true for this directory.
                            deleteEmptyDir(d);
                        }
                ...
                lock (_lock)
                {
                    ...
                    { //file has been created
                        ...
                        { //it was created today
                            if ((file.BaseStream.Length / 1024) <= MAX_FILE_SIZE_KB)
        ...
        private static string GetFileName(string subDirectory, DateTime date, bool singleFileOnly)
        {
            ...
            do
            {
                ...
                if ((fi.Length / 1024) <= MAX_FILE_SIZE_KB)
