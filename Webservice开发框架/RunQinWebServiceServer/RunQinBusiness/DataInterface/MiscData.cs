...
namespace RunQinBusiness.DataInterface
{
    ...
    public class MiscData
    {
        ...
        public static DateTime GetNowDateTime()
        {
            ...
            return SIn.DateT(table.Rows[0][0].ToString());
        }
        ...
        public static void MakeABackup(string serverName = "", string user = "", string pass = "", bool doVerify = false)
        {
            ...
            string oldDb = SIn.String(table.Rows[0][0].ToString());
            ...
            try
            {
                DataConnection dconBackupServerNoTimout = useSameServer ? new DataConnection(newDb) : new DataConnection(serverName, newDb, user, pass, DatabaseType.MySql);
                foreach (DataRow row in table.Rows)
                {
                    ...
                    command = SIn.ByteArray(dtCreate.Rows[0][1]);
                    ...
                    {
                        long count = SIn.Long(dcon.GetCount($"SELECT COUNT(*) FROM `{oldDb}`.`{tableName}`"));
                        int limit = 10000;
                ...
                if (doVerify)
                {
                    ...
                    foreach (DataRow dbTable in table.Rows)
                    {
                        ...
                        int ctOld = SIn.Int(dcon.GetCount($"SELECT COUNT(*) FROM `{oldDb}`.`{tableName}`"));
                        int ctNew = SIn.Int(dconBackupServerNoTimout.GetCount($"SELECT COUNT(*) FROM `{newDb}`.`{tableName}`"));
                        if (ctOld != ctNew)
                        {
        ...
        public static string GenerateTableQuery(string table)
        {
            ...
            string command = "SHOW CREATE TABLE " + SOut.String(table);
        ...
        public static void InsertPreferences(DataTable preferences)
        {
            ...
            foreach (DataRow row in preferences.Rows)
            {
                if (DataConnection.DBtype == DatabaseType.MySql)
                {
                    command = "INSERT INTO preference (PrefName,ValueString) VALUES('" + SOut.String(row[1].ToString()) + "','" + SOut.String(row[2].ToString()) + "')";
                    Db.NonQ(command);
                }
                else
                {//oracle
                    command = "INSERT INTO preference (PrefNum,PrefName,ValueString) "
                        + "VALUES((SELECT COALESCE(MAX(PrefNum),0)+1 FROM preference),'" + SOut.String(row[1].ToString()) + "','" + SOut.String(row[2].ToString()) + "')";
                    Db.NonQ(command);
                }
            }
        }
        ...
        public static int SetMaxAllowedPacket(int sizeBytes)
        {
            ...
            string command = "SET GLOBAL max_allowed_packet=" + SOut.Int(sizeBytes);
            Db.NonQ(command);
        ...
        public static void LockWorkstationsForDbs(string[] dbNames)
        {
            ...
            for (int i = 0; i < dbNames.Length; i++)
            {
                try
                {
                    ...
                    string command = "UPDATE preference SET ValueString ='" + SOut.String(Environment.MachineName)
