...
namespace RunQinBusiness.Remoting
{
    ...
    public class XmlConverter
    {
        ...
        private static object XmlEscapeRecursion(Type type, object obj, ObjectIDGenerator objectIDGenerator)
        {
            ...
            {//DataTable escaping is taken care of in TableToXml
                return obj;
            }
            #endregion
            #region List / Array - IEnumerables
            else if (typeof(IEnumerable).IsAssignableFrom(type))
            {//object is either a list or an array, need to escape all fields and properties of all items
            ...
            else if (!type.IsValueType)
            {
                ...
                foreach (FieldInfo fi in fiArray)
                {
                    try
                    {
                        ...
                        {//if this is a string field, escape for xml
                            if (string.IsNullOrEmpty(objCur.ToString()))
                            {
                                continue;
                            }
                ...
                foreach (PropertyInfo pi in piArray)
                {
                    try
                    {
                        ...
                        {//if this is a string property, escape for xml
                            if (string.IsNullOrEmpty(objCur.ToString()))
                            {
                                continue;
                            }
        ...
        public static DataTable XmlToTable(string xmlData)
        {
            ...
            foreach (XmlNode node in nodeListY)
            {//loop y rows
